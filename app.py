...
# ================== COLOR LOGIC (ปรับปรุงใหม่ให้รองรับสีสว่าง/นีออน) ==================
...
def is_allowed_color(color_hex):
    ...
    # 1. เช็ค Hex ยอดนิยม (เหลือง, ฟ้า) แบบตรงตัว
    if clean_hex in ["ffff00", "00ffff", "cyan", "yellow", "#ffff00", "#00ffff"]:
        return True
    ...
    # 🟡 สีเหลืองสว่าง (Neon/Bright Yellow)
    # หลักการ: แดงและเขียวต้องสูง และต้องมากกว่าน้ำเงินอย่างน้อย 20 หน่วย (ลดจาก 35 เพื่อความง่าย)
    is_yellow = (r > 180 and g > 180) and (r > b + 20 and g > b + 20)
    
    # 🔵 สีฟ้าสว่าง (Bright Cyan)
    # หลักการ: เขียวและน้ำเงินต้องสูง และต้องมากกว่าแดงอย่างน้อย 20 หน่วย
    is_blue = (g > 180 and b > 180) and (g > r + 20 and b > r + 20)
...
def get_district_info(district_name):
    ...
    # ถ้าหาจนจบทุกแถวแล้วไม่เจอแถวที่สีผ่านเงื่อนไข แต่เคยเจอชื่อตรง
    if found_name_match:
...
def handle_message(event):
    ...
    if info:
        if info["status"] == "success":
            ...
            # ตอบกลับกรณีเจอชื่อแต่สีไม่ใช่ (เช่น สีชมพู หรือสีขาว)
            line_bot_api.reply_message(
                event.reply_token,
